...
Eine eigene Zeile `[TOC]` (oder `## [TOC]`) wird zum nativen Confluence-Inhaltsverzeichnis. GitHub-Style Callouts (`> [!NOTE]`, `> [!TIP]`, `> [!IMPORTANT]`, `> [!WARNING]`, `> [!CAUTION]`) werden in native Confluence Info-, Tipp-, Hinweis- und Warn-Makros umgewandelt. Standard GFM-Tabellen werden als Confluence-Tabellen gerendert. Fenced `mermaid`-Flowcharts werden als PlantUML-Makro publiziert.
